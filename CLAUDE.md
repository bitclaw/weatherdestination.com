...
**Nitro itself is no longer a dependency of this project at all** (confirmed: no `nitropack` anywhere in `node_modules`; the `clean` script's `.nitro`/`.output` cleanup targets are dead leftovers, not evidence Nitro is still involved). This project's TanStack Start version builds directly on Vite - no Nitro adapter in the chain - so the original plugin-auto-load bug above is now moot as a live risk. But `server/start.ts` staying a hand-written entry point isn't a Nitro leftover either: it's a still-current tradeoff. TanStack Start's Vite-native build has exactly one built-in mechanism for combined static+SSR serving - `previewServerPlugin` in `@tanstack/start-plugin-core` - and it's scoped to `vite preview` only. It works by registering its SSR handler *after* Vite's own static-file middleware, so prerendered/static files fall through automatically with no hand-maintained routing table; but that plugin has no clustering, no job-worker or Sentry boot hooks, and isn't meant for real production traffic. Since this app needs custom Bun clustering (`cluster.ts`) and startup side effects guaranteed to run, there's no stock TanStack production server to adopt instead (platform adapters exist for Vercel/Cloudflare specifically, not for a self-hosted Bun cluster) - so `server/start.ts` also hand-rolls its own static-vs-SSR request routing (the `PRERENDERED` map + the static-asset branch below it), instead of getting it for free the way `vite preview` does from Vite's static middleware. The flat static-path list itself no longer has to be kept in sync by hand, though - `src/lib/prerendered-paths.ts` (`STATIC_HTML_PATHS`, `NON_HTML_PRERENDERED_PATHS`) is imported by both `vite.config.ts`'s `filter`/`pages` and `server/start.ts`'s `PRERENDERED` map, after the duplicated-list version of this exact bug broke `robots.txt`/`sitemap.xml`. `/blog/*` stays outside that shared list since both files already derive it independently from `content-collections` data.
